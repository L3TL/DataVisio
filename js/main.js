...
function GetQueryStringParams(sParam,defaultVal) {
    var sPageURL = ""+window.location;//.search.substring(1);//Do NOT USE THIS IN SAFARI
...
jQuery.getJSON(GetQueryStringParams("config","config.json"), function(data, textStatus, jqXHR) {
	...
	if (config.type!="network") {
		//This is mainly here just to ensure the config does not corrupt the main file if its crappy
		alert("Invalid configuration settings.")
		return;
	}
	
    //As soon as page is ready (and data ready), the GUI will be set up. 
    //This is mandatory for the panels to be present
...
//====This runs the main graph visualization, based off of the config.json file=====//
function initSigma(config) {
    ...
    //=====This function is to categorize the groups of nodes by color======//
...
//====This is the setup for the interface mentioned earlier=====//
function setupGUI(config) {
	// Initialise main interface elements
	var logo=""; // Logo elements. Technically imported but never used so....yeah
	if (config.logo.file) {
    ...
    //=====THIS IS THE IMPORTANT ONE. DO NOT TINKER WITH THIS SECTION=======//
    //----This section covers the panel that finds the attributes for each node and neighbor---//
	$GP = {
		calculating: !1,
		showgroup: !1
	};
...
//===This function is for the search bar. currently, it can only find nodes based off of name===//
function Search(a) {
    //==hence the name=search===//
...
//===When categorizing the colors earlier, this will hide/show groups based off of color===//
function Cluster(a) {
    this.cluster = a;
    this.display = !1;
...
//==This occurs when the node is selected, it will open the panel, and hide all non-neighbor nodes==//
...
//=======================This is in the event the graph is directed, but I never really used it=====//
	if (groupByDirection) {
...
//====Aforementioned hiding/showing method of colored nodes========//
